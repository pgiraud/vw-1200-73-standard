...
# this is the jekyll root directory!
JEKYLL_ROOT="."
# this is the command used to publish/upload the jekyll _site directory
DEPLOY_CMD="_jekyll_deploy.sh";
DEPLOY_TEST_CMD="_jekyll_deploy_with_draft.sh"
# this is the command to commit the jekyll directory to github
GITPUSH_CMD="cd ..; git pull && git add -A && git commit -m \"Update `date +'%Y.%m.%d %H:%M'`\" && git push; cd $JEKYLL_ROOT; echo \"<enter to continue>\"; read dummy;";
# the post editor
EDITOR="vim";
...
start() {
  ...
  if [ "$argument" != "" ]; then
    ...
    echo "Commands: (<id> | new | draft | toggle <id> | ren <id> | dep | prev | git | q/quit/<enter>)"
    echo -n ":> ";
    read -e -a selection;
  fi
